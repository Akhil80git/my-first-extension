...
// Save folder snapshot (Dynamic)
app.post("/save", (req, res) => {
  const { folderType, files } = req.body;

  if (!folderType || !files) {
    return res.status(400).json({ error: "folderType and files required" });
  }

  const data = readData();

  // Create new folder type automatically
  if (!data[folderType]) {
    data[folderType] = {
      latestVersion: 0,
      versions: []
...
app.get("/fetch/:folderType/:version", (req, res) => {
  ...
  const v = data[folderType].versions.find(item => item.version == version);
